...
function mySetFocus( sLabel )
{
	try
	{
		...
		{	
			...
			if ( c != null )
				/* this built-in setFocus() does not return a value */
				c.setFocus();
		}	
		return;
	}
...
/***************************************************************************************/
/* Need a value on an OnSave event   												   */
function NeedValueOnSave( sLabel1, sLabel2 )
{
	if( DebugModeFmOne ) 
		window.alert( "function NeedValueOnSave()\nsLabel\t" + sLabel );
	try
	{
		if( sLabel1 != null && sLabel2 != null && sLabel2.length > 0 )
		{
			mySetFocus( sLabel2 );
			window.alert( "You must provide a value for " + sLabel1 + "." );
			var a = Xrm.Page.context.getEventArgs();
			if( a != null )
				a.preventDefault();
			else
				window.alert( "function NeedValueOnSave()\nXrm.Page.context.getEventArgs() is null" );
		}
	}
	catch( err )
	{
		if( DebugModeFmOne ) window.alert( "function NeedValueOnSave error code = " + err );
	}
}
